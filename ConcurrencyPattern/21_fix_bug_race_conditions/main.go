...
func (s *sub) loop() {
	...
	for {
		...
		var updates chan Item
		if len(pending) > 0 {
			fmt.Println("len(pending) > 0: get first data")
			first = pending[0]
			updates = s.updates // enable send case
			fmt.Printf("&update = %v  &s.update = %v \n", &updates, &s.updates)
		}
		// STARTFETCHASYNC OMIT
		select {
		case <-startFetch: // HLfetch
			fmt.Println("case <-startFetch: start")
			fetchDone = make(chan fetchResult, 1) // HLfetch
			go func() {
				fmt.Println("case <-startFetch: start s.fetcher.Fetch()")
				fetched, next, err := s.fetcher.Fetch()
				fmt.Println("case <-startFetch: enable fetchDone")
				fetchDone <- fetchResult{fetched, next, err}
				fmt.Println("case <-startFetch: done s.fetcher.Fetch()")
			}()
			fmt.Println("case <-startFetch: end ")
		case result := <-fetchDone: // HLfetch
			fmt.Println("case result := <-fetchDone: start")
			...
			if err != nil {
				fmt.Println("have error, sleep 10 s")
				next = time.Now().Add(10 * time.Second)
				break
			}
			fmt.Println("case result := <-fetchDone: start merger data to pending map")
			for _, item := range fetched {
			...
			fmt.Println("case result := <-fetchDone: end")
		case errc := <-s.closing:
			fmt.Println("case errc := <-s.closing: start ")
			errc <- err
			fmt.Println("case errc := <-s.closing: close data in channel s.update")
			close(s.updates)
			fmt.Println("case errc := <-s.closing: end")
			return
		case updates <- first:
			fmt.Println("start case updates <- first: start")
			fmt.Println("start case updates <- first: update again pending")
			pending = pending[1:]
			fmt.Println("end case updates <- first: end")
		}
	}
}
