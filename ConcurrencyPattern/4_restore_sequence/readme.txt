/*
ideal:
 it's same job in 3_generator, but we want when we need data, we will get it, and not, routine is pending.
 we have flag share read and write data.
    ...
    every time after get data from data stream, we push one data to flag channel, it's fuel (flag1, flag2, flag3, ...) for all routine stream continue.
