...
/**
  ...
  +) chanZero block Sends and Receive, available when send or receive ready
     un cmt line 23 ->to 44 : we have two routine Interactive with channel zero ( test.cZero )
     run and view result:
     ...
     in line  41,42, have two cmd write to channel : test.cZero <- 1, but it's blocked why not receive ready
	...
	read first from channel One with empty
		...
		read=  1
    same case one, we have 2 routine action to routine one test.cOne
