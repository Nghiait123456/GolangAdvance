...
Methods with value receivers can be called on pointers as well as values. Methods with pointer receivers can only be called on pointers or addressable values
  ...
  if uncmt linr 24 sVals[1].Write("24_cleary_example_smart_fetch"), you run code have error:
  ...
  in line 29, sPtrs[1].Write("24_cleary_example_smart_fetch"), Write() is method with pointer receive. It's run success with pointer or address receive
