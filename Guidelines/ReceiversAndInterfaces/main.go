...
Methods with value receivers can be called on pointers as well as values. Methods with pointer receivers can only be called on pointers or addressable values
  ...
  if uncmt linr 24 sVals[1].Write("test"), you run code have error:
  ...
  in line 29, sPtrs[1].Write("test"), Write() is method with pointer receive. It's run success with pointer or address receive
